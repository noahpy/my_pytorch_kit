...
from my_pytorch_kit.model.vae import ImageVAEwithAE
...
if __name__ == '__main__':
    ...
    if train:
        ...
        if load_model:
            path = input("Enter path to model (default: models/vae_with_ae.pt) :")
            if path == "":
                path = "models/vae_with_ae.pt"
            model.load_model(path)

        trainer.train(optimizer, **hparams)

        model.save_model("models/vae_with_ae.pt", hparams)

    else:
        model.load_model("models/vae_with_ae.pt")
